  ...
  <div class="dialog-container">
    ...
    >
      <div class="opration-stop">
        <span class="name">止盈價</span>
        <span class="pro-state">{{ currentWinSet }}</span>
        <p class="input-box">
          <input
            v-model="setPercent"
            class="input-text"
...
export default {
  ...
  methods: {
    ...
    enterStop() {
      if (!this.setPercent) {
        this.$mesXCsage.error('Please enter the profit stop price');
        return;
      }
      assetsApi
        .stopWinConfig(this.orderId, this.setPercent, this.payPass)
        .then((res) => {
          if (res.code === '200') {
            this.setPercent = '';
