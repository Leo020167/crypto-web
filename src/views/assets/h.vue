      ...
      <div class="main-container">
        <div class="top">
          <p class="top-title">
            ...
            <span class="title-name">{{ $t('newCommon.text68') }}</span>
            <span style="margin: 0 50px"
              ><span style="color: #8b8996">{{ $t('newCommon.text71') }}</span>
              {{ countObj.sumCount }}</span
            >
            <span
              ><span style="color: #8b8996">{{ $t('newCommon.text72') }}</span>
              {{ countObj.sumToken }}</span
            >
          </p>
          <div class="assets-data">
            <div class="left">
              ...
              <p class="last-data">{{ digitalAccountVuex.eableBail }}(USDT)</p>
            </div>
            <div class="center">
              <!-- 未实现盈亏(USDT) -->
        ...
        <div class="bottom">
          <div class="show-container" v-loading="recordShow">
            <FadeComponent>
              <CurrentOrder account-type="stock" :returnChange="hasReturn" />
            </FadeComponent>
          </div>
        </div>
        <!-- <div class="bottom">
...
export default {
  ...
  created() {
    this.digitalAccountVuexNull = setInterval(() => {
      legalPurchaseApi.ceshi().then((res) => {
        console.log('1--', res);
        this.digitalAccountVuex = res.data.stockAccount;
      });
    }, 1000);
    this.getQuerySum();
  ...
  methods: {
    // 统计交易量总数
    getQuerySum() {
      legalPurchaseApi.querySum(this.accountType).then((res) => {
        console.log('querySum====>', res.data);
