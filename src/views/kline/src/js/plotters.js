    ...
    Draw(context) {
        ...
        for (let i = start; i < last; i++) {
            ...
            if (close > open) {
                ...
                if (iH > 1 && iW > 1 && dark)
                    strokePosRects.push({x: left, y: top, w: Math.max(iW, 1), h: Math.max(iH, 1)});
                else
        ...
        for (let i = first; i < last; i++) {
            ...
            if (MACD >= 0) {
                ...
                if ((i === 0 || MACD >= prevMACD) && iH > 1 && iW > 1)
                    strokePosRects.push({x: left, y: middle - iH, w: Math.max(iW, 1), h: Math.max(iH, 1)});
                else
                    fillPosRects.push({x: left, y: middle - iH, w: Math.max(iW, 1), h: Math.max(iH, 1)});
            } else {
                let iH = range.toHeight(-MACD);
                if ((i === 0 || MACD >= prevMACD) && iH > 1 && iW > 1)
                    strokeNegRects.push({x: left, y: middle, w: Math.max(iW, 1), h: Math.max(iH, 1)});
                else
