      ...
      <el-table-column label="交易对">
        <template slot-scope="scope">
          <font-awesome-icon
            ...
            :class="[
              'icon-star',
              optionals.includes(scope.row.symbol) ? 'icon-star-on' : '',
            ]"
            @click="handleAddOrCancelOptional(scope.row.symbol)"
          />
  ...
  methods: {
    ...
    handleAddOrCancelOptional(symbol) {
      ...
      if (this.optionals.includes(symbol)) {
        this.$store
          .dispatch('cancelMyOptionals', symbol)
          .then(() => {
            this.getMyCoin();
          })
          .catch(() => {
        ...
        // 添加自選
        this.$store
          .dispatch('addMyOptionals', symbol)
          .then(() => {
            this.getMyCoin();
          })
          .catch(() => {
